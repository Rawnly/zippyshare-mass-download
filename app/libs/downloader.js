...
module.exports = async function (
	...
	spinner.start(
		chalk `{dim [ETA: calculating...]} {yellow ${urls.length + files.length - 1}} of {yellow ${source.length}}`
	);

	for (let i = files.length - 1; i < source.length; i++) {
		const element = source[i].replace(/\s+/, "");
